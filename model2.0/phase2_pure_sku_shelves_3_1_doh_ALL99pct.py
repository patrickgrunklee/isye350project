"""
PHASE 2 WITH PURE-SKU PACKING (DISCRETE + FURNITURE CONTINUOUS)
================================================================

Key Changes:
1. Pure-SKU shelves (most SKUs): Discrete item-based packing (respects max items/shelf)
2. Pure-SKU shelves (chairs & desks): Continuous volume-based packing (no item limits)
3. Mixed-SKU shelves: Follow discrete item structure (7/6/4 items depending on facility)
4. DoH: Traditional safety stock calculation

Pure-SKU Capacity Calculation:
  - Most SKUs: Packages_per_item × Max_items_per_shelf
    Example: Columbus Pallet (7 items/shelf)
      If 9 packages fit per item → 9 × 7 = 63 packages/shelf
  - Furniture (SKUC1, SKUC2, SKUD1, SKUD2, SKUD3):
    Continuous packing → floor(shelf_volume / package_volume)
"""
...
print("="*100)
print("PHASE 2: PURE-SKU PACKING MODEL (DISCRETE + FURNITURE CONTINUOUS) - 3/1 DAYS-ON-HAND")
print("="*100)
print("\nAPPROACH:")
print("  - Pure-SKU shelves (most SKUs): Discrete item-based packing")
print("  - Pure-SKU shelves (furniture): Continuous packing (volume/weight only)")
print("  - Mixed-SKU shelves: Discrete item structure")
print("  - DoH = 3 days (International) / 1 day (Domestic)")
print("  - Traditional safety stock calculation")
print()
...
# Force ALL facilities to 99%+ pallet utilization if ANY facility needs expansion
# This ensures we maximize existing capacity before expanding anywhere

total_expansion_slack = Sum(st, slack_shelf_sac[st]) + Sum(st, slack_shelf_austin[st])

# Columbus at 99%+ if any expansion needed
...
columbus_utilization[...] = (
    ...
    # If any expansion slack > 0, Columbus must be at 99%+
)

# Sacramento at 99%+ if any expansion needed (including Austin)
...
sacramento_utilization[...] = (
    ...
    1000000 * (1 - total_expansion_slack / 1000000)
    # If any expansion slack > 0, Sacramento must be at 99%+
)

# Austin at 99%+ if Austin itself needs expansion OR if Sacramento needs expansion
...
austin_utilization[...] = (
    ...
    1000000 * (1 - total_expansion_slack / 1000000)
    # If any expansion slack > 0, Austin must be at 99%+
)

print("   ✓ Constraints created (ALL facilities must be at 99%+ utilization if any expansion needed)")
...
print("\n[2] FACILITY UTILIZATION & EXPANSION REQUIREMENTS")

# Calculate utilization for all facilities
...
col_util = facility_utilization['Columbus']
print(f"  Pallet shelves used: {col_util['used']:,.0f} / {col_util['capacity']:,} ({col_util['pct']:.1f}% utilization)")
if col_util['pct'] >= 99:
    print(f"  ✓ At 99%+ capacity")
else:
    print(f"  ⚠️  Only at {col_util['pct']:.1f}% - not fully utilized")

print(f"\nSacramento:")
sac_util = facility_utilization['Sacramento']
print(f"  Pallet shelves used: {sac_util['used']:,.0f} / {sac_util['capacity']:,} ({sac_util['pct']:.1f}% utilization)")
if sac_util['pct'] >= 99:
    print(f"  ✓ At 99%+ capacity")
else:
    print(f"  ⚠️  Only at {sac_util['pct']:.1f}% - not fully utilized")

print(f"\nAustin:")
aus_util = facility_utilization['Austin']
print(f"  Pallet shelves used: {aus_util['used']:,.0f} / {aus_util['capacity']:,} ({aus_util['pct']:.1f}% utilization)")
if aus_util['pct'] >= 99:
    print(f"  ✓ At 99%+ capacity")
else:
    print(f"  ⚠️  Only at {aus_util['pct']:.1f}% - not fully utilized")


print("\n" + "-" * 100)
print("EXPANSION REQUIREMENTS")
print("-" * 100)
