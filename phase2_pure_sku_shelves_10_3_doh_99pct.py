...
print("  - International: 10 business days | Domestic: 3 business days")
print()
...
# Force Columbus to 99%+ utilization if Sacramento or Austin need expansion
...
columbus_utilization[...] = (
    ...
    1000000 * (1 - (Sum(st, slack_shelf_sac[st]) + Sum(st, slack_shelf_austin[st])) / 1000000)
    # If any slack at Sac/Austin (slack > 0), then Columbus must be at 99%+ utilization
    # Big-M formulation: If total_slack = 0, constraint is relaxed
)

print("   ✓ Constraints created (including Columbus 99% utilization requirement)")
...
# Columbus first - show utilization
...
columbus_pallet_used = 0
for _, row in shelves_used_df.iterrows():
    config_id = int(float(row['Config_ID']))
    shelves = row['Shelves']
    if config_facility[config_id] == 'Columbus' and config_storage_type[config_id] == 'Pallet':
        columbus_pallet_used += shelves

columbus_pallet_capacity = curr_shelves.get(('Columbus', 'Pallet'), 0)
utilization_pct = (columbus_pallet_used / columbus_pallet_capacity * 100) if columbus_pallet_capacity > 0 else 0

print(f"  Pallet shelves used: {columbus_pallet_used:,.0f} / {columbus_pallet_capacity:,} ({utilization_pct:.1f}% utilization)")
if utilization_pct >= 99:
    print(f"  ✓ Columbus at 99%+ capacity (maximizing existing resources)")
else:
    print(f"  ⚠️  Columbus only at {utilization_pct:.1f}% - not fully utilized")
