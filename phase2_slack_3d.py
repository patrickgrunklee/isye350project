"""
PHASE 2 WITH SLACK - Using 3D Bin Packed Configurations
========================================================

Uses the corrected Phase 1 3D bin packing configurations
"""
...
print("="*100)
print("PHASE 2 SLACK ANALYSIS - WITH 3D BIN PACKED CONFIGURATIONS")
print("="*100)
print("\nUsing corrected Phase 1 configurations with multiple packages per item\n")
...
# Create mapping: (Config_ID, SKU) -> Total_Packages_per_Shelf (already includes items × packages/item)
config_units = {}
config_facility = {}
config_storage_type = {}
...
for _, row in packing_configs_df.iterrows():
    ...
    sku = row['SKU']
    total_packages = int(row['Total_Packages_per_Shelf'])  # This is the 3D packed result
    fac = row['Facility']
    st = row['Storage_Type']
    units_per_package = int(row['Units_per_Package'])

    # Total units per shelf for this config
...
print("\n   Sample 3D configurations:")
for config_id in [8, 11, 44, 47]:  # Show some pallet examples
    if config_id in config_facility:
        fac = config_facility[config_id]
        st = config_storage_type[config_id]
        sku_list = [(sku, units) for sku, units in config_units[config_id].items()]
        print(f"     Config {config_id}: {fac} {st} - {sku_list}")

print("\n[7/6] Building GAMSPy model with slack variables...")
...
slack_capacity = Variable(m, name="slack_capacity", domain=[t, s, f], type="positive")
...
obj[...] = (
    ...
    Sum([t, s, f], slack_capacity[t, s, f] * 1) +
...
# Solve
print("\n" + "="*100)
print("SOLVING MODEL WITH 3D CONFIGURATIONS")
print("="*100)

slack_model = Model(
    m,
    name="phase2_slack_3d",
...
print("\n" + "="*100)
print("SLACK VARIABLE ANALYSIS - WITH 3D BIN PACKING")
print("="*100)
...
print("="*100)
print("\nWith 3D bin packing, we can fit 2-16× more packages per shelf!")
print("This should dramatically reduce shelf requirements.")
